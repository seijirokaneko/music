...
dir = "songs/";
playlist = ["Elektronomia - Limitless", "Culture Code - Feel Again (feat. Harley Bird)", "Julius Dreisig & Zeus X Crona - Invisible", "Fransis Derelle - Fly (feat. Parker Pohill)", "Tobu - Sound of Goodbye"];

title = ["Limitless", "Feel Again", "Invisible", "Fly", "Sound of Goodbye"];
poster = ["images/1.jpg", "images/2.jpg", "images/3.jpg", "images/4.jpg", "images/5.jpg"];

artists = ["Elektronomia", "Culture Code", "Julius Dreisig & Zeus X Crona", "Fransis Derelle", "Tobu"];
...
let list = document.querySelector('input[name=lists]');
let listbtn = document.querySelector('.fa-bars');
list.addEventListener('change', function() {
    if (this.checked) {
        document.querySelector(".lists").classList.add("active");
        listbtn.classList.replace("fa-bars", "fa-times");
    } else {
        document.querySelector(".lists").classList.remove("active");
        listbtn.classList.replace("fa-times", "fa-bars");
    }
});
...
title.forEach((v, i) => {
    ...
    let songttl = lists.appendChild(li);
    songttl.value = i;
    songttl.innerHTML = v;
})
